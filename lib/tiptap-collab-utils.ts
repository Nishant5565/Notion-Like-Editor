...
export const fetchCollabToken = async () => {
  ...
  // Return null if no token is available for local-only usage
  if (!TIPTAP_COLLAB_TOKEN) {
    console.info("No collaboration token provided - running in local-only mode")
    return null
  }
...
export const fetchAiToken = async () => {
  ...
  // Return null if no token is available for local-only usage
  if (!TIPTAP_AI_TOKEN) {
    console.info("No AI token provided - running without AI features")
    return null
  }
